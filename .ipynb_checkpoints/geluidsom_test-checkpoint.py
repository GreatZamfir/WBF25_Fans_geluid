...
class room_coeffient(unittest.TestCase):
    def setUp(self):
        self.oppervlaktes = a([1.0, 2.0])
        self.absorpties = a([
            [0.2, 0.4],
            [0.4, 0.8],
            [0.0, 0.0]
        ])
        self.antwoord = a([ 1.5, 6.0,  0.0 ])

    def test_input_array(self):
        result = gs.room_coefficient(self.oppervlaktes, self.absorpties)
        expected = self.antwoord
        assert_allclose(result, expected)
    
    def test_input_array_foute_dimensies(self):
        absorb_fout = self.absorpties[:, 0]
        with self.assertRaises(ValueError):
            result = gs.room_coefficient(self.oppervlaktes, absorb_fout)
        
    def test_input_dataframe(self):
        series_opp = pd.Series(
            data= self.oppervlaktes, 
            index = ['eerste', 'tweede']
        )
        df_absorpties = pd.DataFrame(
            data = self.absorpties, 
            columns = ['eerste', 'tweede'], 
            index = [63, 125, 250] 
        )
        result = gs.room_coefficient(series_opp, df_absorpties)
        expected = pd.Series (
            data =a([ 1.5, 6.0,  0.0 ]),
            index = [63,125,250]
        )
